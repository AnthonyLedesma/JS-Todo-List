...
const render = (ToDoArray) => {
  const parentClass = 'uk-grid-small uk-child-width-expand@s uk-text-center uk-flex';
  const badgeClass = 'uk-card-badge';
  let bodyPriorityClass = '';
  let priorityState = '';
  const gridClass = 'uk-grid-small uk-child-width-expand@s uk-text-center uk-flex';
  const editClass = 'uk-button uk-button-text uk-align-right';


  preRenderClear();
  ToDoArray.forEach((element, index) => {
    if (parseInt(element.priority, 10) === 0) { bodyPriorityClass = 'uk-card uk-card-default uk-card-body uk-card-hover uk-margin'; priorityState = 'Low'; } else if (parseInt(element.priority, 10) === 1) { bodyPriorityClass = 'uk-card uk-card-primary uk-card-body uk-card-hover uk-margin'; priorityState = 'Normal'; } else if (parseInt(element.priority, 10) === 2) { bodyPriorityClass = 'uk-card uk-card-secondary uk-card-body uk-card-hover uk-margin'; priorityState = 'High'; }
    $('#ToDoList').append(`
          <div class="${gridClass}">
              <div>
    ...
    */
    ...
    $(`#delete${index}`).click((e) => {
      e.preventDefault();
      e.target.blur();
      UIkit.modal.prompt(`Type <b>"remove"</b> To Remove To-Do Item <b>"${e.target.parentNode.parentNode.parentNode.firstElementChild.textContent}"</b>`, '').then((answer) => {
        if (answer === 'remove') {
          deleteMode(index, ToDoArray);
          localStorage.setItem('ToDoArray', JSON.stringify(ToDoArray));
          render(ToDoArray);
        }
      });
    ...
    $(`input[name="radio${index}"][data-value="${parseInt(element.priority, 10)}"]`).prop('checked', true);
